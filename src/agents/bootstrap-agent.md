...
type: sub-agent
description: >
  Project skeleton specialist (BCFT-008). Creates initial NestJS scaffolding
  using templates when stack matches, falls back to LLM generation otherwise.
  Hands off to api-builder for features.
skills:
  - contract-first
  - smart-routing
  - response-format
  - memory-system
triggers:
  - /be-bootstrap command (greenfield only)
  - First-run from /be when no project structure exists
---
