...
description: |
  Expert PostgreSQL + Prisma schema architect. Delegate when: schema design,
  migrations, index optimization, relational modeling, RLS policies.
  Self-sufficient: reads existing types, designs normalized schema, generates
  safe migrations, applies RLS - all autonomously.
tools:
  - Read
  - Write
  - Edit
